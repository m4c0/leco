...
static void find_android_llvm(sim_sb *res) {
  ...
  for (auto e : pprent::list(res->buffer)) {
    if (strcmp(max.buffer, e) > 0) continue;
  ...
  if (!exists(res)) sys::die("prebuilt path isn't a directory: [%s]", res->buffer);
...
static const char *wasm_sysroot() { return sys::env("WASI_SYSROOT"); }
...
int main(int argc, char **argv) try {
  ...
  if (mtime::of(cf.buffer) > 0) {
    ...
    if (fgets(buf.buffer, buf.size, f) != nullptr) {
      if (!quiet) fwrite(buf.buffer, 1, buf.size, stdout);
      return 0;
    }
    fclose(f);
  }
